...
`tvl-validate`, `tvl-measure-validate`, and `tvl-microsim-bridge` had zero
tests exercising the CLI entry point itself (argparse wiring, exit code,
`--json`/`--format json` payload shape, error path) — only their underlying
library functions were unit-tested elsewhere, or not at all. The other six
(`tvl-lint`, `tvl-ci-gate`, `tvl-config-validate`, `tvl-compose`,
`tvl-check-structural`, `tvl-check-operational`) already have CLI-level tests
in `test_cli_error_paths.py`, `test_lint_non_mapping.py`,
`test_tvl_compose.py`, `test_constraints_parser_fixes.py`, and
`test_parser_dos_guards.py`.

This file closes the gap for the remaining four, at a smoke level: one
success path (exit 0, well-shaped `--json`/`--format json` output) and one
error path (missing input file) per CLI, documenting the exit code each
